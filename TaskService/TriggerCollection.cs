...
namespace Microsoft.Win32.TaskScheduler
{
	...
	public sealed class TriggerCollection : IList<Trigger>, IDisposable, IXmlSerializable, IList//, Models.ITriggerCollection
	{
		...
		/*/// <summary>
		/// Add an unbound <see cref="Trigger"/> to the task. </summary> <param name="unboundTrigger"><see cref="Trigger"/> derivative to add
		/// to the task.</param> <returns>Bound trigger.</returns> <exception cref="System.ArgumentNullException"><c>unboundTrigger</c> is <c>null</c>.</exception>
		...
		{
			if (v1Task != null)
				return Trigger.CreateTrigger(v1Task.CreateTrigger(out var idx), Trigger.ConvertToV1TriggerType(taskTriggerType));
			return Trigger.CreateTrigger(v2Coll.Create((TASK_TRIGGER_TYPE2)taskTriggerType), v2Def);
		}
		...
		/// <summary>
		/// Copies the elements of the <see cref="TriggerCollection"/> to a <see cref="Trigger"/> array, starting at a particular
		/// <see cref="Trigger"/> array index.
		/// </summary>
		/// <param name="index">The zero-based index in the source at which copying begins.</param>
		/// <param name="array">
		/// The <see cref="Trigger"/> array that is the destination of the elements copied from <see cref="TriggerCollection"/>. The
		/// <see cref="Trigger"/> array must have zero-based indexing.
		/// </param>
		...
		/// otherwise, false. This method also returns false if <paramref name="item"/> is not found in the original <see cref="ICollection{T}"/>.
		/// </returns>
		...
		/// <summary>Returns a <see cref="System.String"/> that represents the triggers in this collection.</summary>
