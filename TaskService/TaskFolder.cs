...
namespace Microsoft.Win32.TaskScheduler
{
	...
	public sealed class TaskFolder : IDisposable, IComparable<TaskFolder>//, Models.ITaskFolder
	{
		...
		/// </summary>
		/// <value>A <see cref="System.Collections.Generic.IEnumerator{Task}"/> for all <see cref="Task"/> instances.</value>
		[NotNull, ItemNotNull]
		public IEnumerable<Task> AllTasks => EnumerateFolderTasks(this);

		/// <summary>Gets the parent folder of this folder.</summary>
		/// <value>The parent folder, or <c>null</c> if this folder is the root folder.</value>
...
#pragma warning restore 0618
		...
		/// <summary>Gets a collection of all the tasks in the folder.</summary>
		[NotNull, ItemNotNull]
		...
		/// <summary>Creates a folder for related tasks. Not available to Task Scheduler 1.0.</summary>
		/// <param name="subFolderName">
		/// The name used to identify the folder. If "FolderName\SubFolder1\SubFolder2" is specified, the entire folder tree will be created
		/// if the folders do not exist. This parameter can be a relative path to the current <see cref="TaskFolder"/> instance. The root
		/// task folder is specified with a backslash (\). An example of a task folder path, under the root task folder, is \MyTaskFolder.
		/// The '.' character cannot be used to specify the current task folder and the '..' characters cannot be used to specify the parent
		...
		public TaskFolder CreateFolder([NotNull] string subFolderName, string sddlForm = null, bool exceptionOnExists = true)
		{
			...
			{
				var serr = ce.ErrorCode & 0x0000FFFF;
				if (serr == 0xb7) // ERROR_ALREADY_EXISTS
				{
					...
					try
					{
						...
						{
							var sd = ifld.GetSecurityDescriptor((Vanara.PInvoke.SECURITY_INFORMATION)Task.defaultSecurityInfosSections);
		...
		/// <summary>Deletes a subfolder from the parent folder. Not available to Task Scheduler 1.0.</summary>
		/// <param name="subFolderName">
		...
		/// path to the folder you want to delete. An example of a task folder path, under the root task folder, is \MyTaskFolder. The '.'
		...
		/// <param name="name">
		/// The name of the task that is specified when the task was registered. The '.' character cannot be used to specify the current task
		...
		/// described by the current <see cref="TaskFolder"/> object.
		/// </summary>
		...
		/// <summary>
		/// Gets a <see cref="TaskSecurity"/> object that encapsulates the specified type of access control list (ACL) entries for the task
		/// folder described by the current <see cref="TaskFolder"/> object.
		/// </summary>
		...
		{
			if (v2Folder != null)
				return v2Folder.GetSecurityDescriptor((Vanara.PInvoke.SECURITY_INFORMATION)includeSections);
			throw new NotV1SupportedException();
		}

		/// <summary>Gets a collection of all the tasks in the folder whose name matches the optional <paramref name="filter"/>.</summary>
		...
		{
			if (v2Folder != null)
				return new TaskCollection(this, v2Folder.GetTasks(TASK_ENUM_FLAGS.TASK_ENUM_HIDDEN), filter);
		...
		/// descriptor for a task in order to allow or deny certain users and groups access to a task.
		/// </param>
		/// <returns>A <see cref="Task"/> instance that represents the new task.</returns>
		/// <example>
		/// <code lang="cs">
		...
		{
			if (v2Folder != null)
				return Task.CreateTask(TaskService, v2Folder.RegisterTask(path, xmlText, (TASK_CREATION)createType, userId, password, (TASK_LOGON_TYPE)logonType, sddl));
		...
		/// <summary>Registers (creates) a task in a specified location using a <see cref="TaskDefinition"/> instance to define a task.</summary>
		/// <param name="path">
		/// The task name. If this value is NULL, the task will be registered in the root task folder and the task name will be a GUID value
		/// that is created by the Task Scheduler service. A task name cannot begin or end with a space character. The '.' character cannot
		/// be used to specify the current task folder and the '..' characters cannot be used to specify the parent task folder in the path.
		...
		/// </param>
		/// <returns>
		/// A <see cref="Task"/> instance that represents the new task. This will return <c>null</c> if <paramref name="createType"/> is set
		/// to <c>ValidateOnly</c> and there are no validation errors.
		/// </returns>
		/// <exception cref="System.ArgumentOutOfRangeException">
		/// Task names may not include any characters which are invalid for file names. or Task names ending with a period followed by three
		/// or fewer characters cannot be retrieved due to a bug in the native library.
		/// </exception>
		/// <exception cref="NotV1SupportedException">
		/// This LogonType is not supported on Task Scheduler 1.0. or Security settings are not available on Task Scheduler 1.0. or
		/// Registration triggers are not available on Task Scheduler 1.0. or XML validation not available on Task Scheduler 1.0.
		/// </exception>
		/// <remarks>
		/// This method is effectively the "Save" method for tasks. It takes a modified <c>TaskDefinition</c> instance and registers it in
		/// the folder defined by this <c>TaskFolder</c> instance. Optionally, you can use this method to override the user, password and
		/// logon type defined in the definition and supply security against the task.
		/// </remarks>
		/// <example>
		...
		public Task RegisterTaskDefinition([NotNull] string path, [NotNull] TaskDefinition definition, TaskCreation createType, string userId, string password = null, TaskLogonType logonType = TaskLogonType.S4U, string sddl = null)
		{
			...
			if (v2Folder != null)
			{
				...
				// The following line compensates for an omission in the native library that never actually sets the registration date
				// (thanks ixm7).
				if (definition.RegistrationInfo.Date == DateTime.MinValue) definition.RegistrationInfo.Date = DateTime.Now;
				var iRegTask = v2Folder.RegisterTaskDefinition(path, definition.v2Def, (TASK_CREATION)createType, userId ?? user.Name, password, (TASK_LOGON_TYPE)logonType, sddl);
			...
			var invChars = Regex.Escape(new string(System.IO.Path.GetInvalidFileNameChars()));
			...
			switch (logonType)
			{
				...
				case TaskLogonType.InteractiveToken:
					flags |= (TaskFlags.TASK_FLAG_RUN_ONLY_IF_LOGGED_ON | TaskFlags.TASK_FLAG_INTERACTIVE);
					definition.v1Task.SetAccountInformation(user.Name, IntPtr.Zero);
					break;

				case TaskLogonType.ServiceAccount:
					flags &= ~(TaskFlags.TASK_FLAG_INTERACTIVE | TaskFlags.TASK_FLAG_RUN_ONLY_IF_LOGGED_ON);
				...
				case TaskLogonType.InteractiveTokenOrPassword:
					flags |= TaskFlags.TASK_FLAG_INTERACTIVE;
					using (var cpwd = new SafeCoTaskMemString(password))
		...
		/// <see cref="TaskFolder"/> object.
		/// </summary>
		/// <param name="taskSecurity">
		/// A <see cref="TaskSecurity"/> object that describes an access control list (ACL) entry to apply to the current folder.
		/// </param>
		public void SetAccessControl([NotNull] TaskSecurity taskSecurity) => taskSecurity.Persist(this);

		/// <summary>Sets the security descriptor for the folder. Not available to Task Scheduler 1.0.</summary>
